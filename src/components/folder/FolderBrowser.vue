...
<template>
  ...
  <div class="flex flex-col">
    ...
    <button class="self-end" @click.prevent="view = view == 'tabular' ? 'gallery' : 'tabular'">
    ...
    <table class="table-auto box-border" v-if="view == 'tabular'">
      ...
      <tbody>
        ...
        <tr v-for="content in contents" :key="content.id" class="odd:bg-white even:bg-slate-50 text-slate-600">
          <td class="pl-2" v-if="selectActions"><input :checked="selected.has(content.id)" @click="$emit('select-content', content, $event)" type="checkbox" /></td>
