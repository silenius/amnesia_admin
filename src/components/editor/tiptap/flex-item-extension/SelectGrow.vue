<template>
  <Listbox as="div" v-model="grow">
    <ListboxButton class="font-bold border rounded-full p-2 w-full">{{ grow }}</ListboxButton>
    <ListboxOptions :class="class_opts">
      <ListboxOption v-for="w in all_grow" :key="w" :value="w">
...
const all_grow = computed(
  () => props.extension.options.grows.toSpliced(0, 0, undefined)
)
...
const grow = computed({
  ...
  get() {
    try {
      return getSelectedAttrs(props, 'grow')
    } catch (e) {
      return 'none'
    }
  },
