<template>
  <Listbox as="div" v-model="direction">
    <ListboxButton class="font-bold border rounded-full p-2 w-full">{{ direction }}</ListboxButton>
    <ListboxOptions :class="class_opts">
      <ListboxOption v-for="w in directions" :key="w" :value="w">
        <button :class="class_opt">{{ w !== undefined ? w : 'none'}}</button>
      </ListboxOption>
    </ListboxOptions>
  </Listbox>
</template>

<script setup>
import { computed } from 'vue'

import { getSelectedAttrs, props_extension } from '../utils'

import {
  Listbox,
  ListboxButton,
  ListboxOptions,
  ListboxOption,
} from '@headlessui/vue'

const props = defineProps(props_extension)
const emits = defineEmits(['select-direction'])

const class_opts = [
  'absolute', 'text-center', 'max-h-48', 'rounded-sm', 'text-black', 'bg-white', 'overflow-scroll', 'z-10'
]
const class_opt = [
  'px-4', 'hover:bg-slate-800', 'w-full', 'hover:text-white'
]

const directions = computed(
  () => props.extension.options.directions.toSpliced(0, 0, undefined)
)

const direction = computed({

  get() {
    try {
      return getSelectedAttrs(props, 'direction')
    } catch (e) {
      return 'none'
    }
  },

  set(value) { 
    return emits('select-direction', {
      direction: value, 
      breakpoint: props.breakpoint, 
    })
  }

})

</script>
